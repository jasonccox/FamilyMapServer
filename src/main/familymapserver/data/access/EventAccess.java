...
public class EventAccess extends Access {
    ...
    private static final String CREATE_STMT = "CREATE TABLE event (" + 
    ...
    public boolean add(Event event) throws DBException {
        ...
        String sql = "INSERT INTO event (id, assoc_username, person_id, latitude, longitude, country, city, type, year) " +
                     "SELECT ?, ?, ?, ?, ?, ?, ?, ?, ? " +
    ...
    public Event get(String eventId) throws DBException {
        ...
        String sql = "SELECT id, assoc_username, person_id, latitude, longitude, country, city, type, year " +
                     "FROM event " +
                     "WHERE id = ?";
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            event = new Event(rs.getString(1), rs.getString(2), rs.getString(3), rs.getFloat(4),
    ...
    public Collection<Event> getAll(String username) throws DBException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                events.add(new Event(rs.getString(1), rs.getString(2), rs.getString(3), rs.getFloat(4),
                                     rs.getFloat(5), rs.getString(6), rs.getString(7), rs.getString(8), 
                                     rs.getInt(9)));
            }
        } catch (SQLException e) {
